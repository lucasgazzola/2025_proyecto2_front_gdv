...
export default function EditCategoryModal({
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof z.infer<typeof categorySchema>, string>>
  >({});
  ...
  const handleChange = (
    ...
    setErrors((p) => {
      const newErrors = { ...p } as Partial<
        Record<keyof z.infer<typeof categorySchema>, string>
      >;
      delete newErrors[name as keyof z.infer<typeof categorySchema>];
      return newErrors;
    });
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!parsed.success) {
      ...
      parsed.error.issues.forEach((issue) => {
        fieldErrors[issue.path[0] as keyof z.infer<typeof categorySchema>] =
          issue.message;
      });
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <DialogDescription className="text-muted-foreground text-sm">
                {isEdit ? "Modifica la categoría" : "Crea una nueva categoría"}
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
